...
```
你胡了 · 平胡 1 分 · 对家点炮                  第 1 行：谁胡了 · 牌型几分 · 谁点炮（自摸就写「自摸」）
翻5万 → 翻牌鸡 6万（1 分/张）                  第 2 行：翻到什么 → 翻牌鸡是哪张、几分/张
清风雀神［叫牌］闷豆6条(自)（鸡 0 分）         第 3~6 行：自己 → 对家 → 上家 → 下家
对家［叫牌］幺鸡×1 乌骨鸡×1（鸡 2 分）
上家［叫牌］幺鸡×2 乌骨鸡×2（鸡 4 分）
下家［未叫］打 幺鸡×1（鸡 0 分） → 包鸡 各 1 分
豆：闷豆6条(自) 三家各付 3 分                  再往下：豆 · 冲锋鸡 · 责任鸡 · 未摸到的牌 · 本局 / 累计
```

读的时候只要盯四处：

1. **［叫牌］/［未叫］** —— 只有叫牌家能收；未叫家只付不收。
2. **牌种后面跟的数量** —— `幺鸡(金)×2` 是金幺鸡 2 张、`6万(翻牌鸡)×1` 是翻牌鸡 1 张；
   **未叫家那一行每项前面带 `-`**，表示这些**不算他的分**。
3. **「打」这个标记** —— 没标记的是他手里 + 副露的，带「打」的是他打出去的；
   未叫家打出的 幺鸡 / 8筒 才进包鸡（那一行尾巴写 `→ 包鸡 各 N 分`）。
4. **括号里的「鸡 N 分」** —— 那一家的**鸡**值多少分；杠（豆腐）的收付在下面「豆」那一行，
   写法是 `闷豆6条(自)`、`点豆5万(上)`（括号里是**谁放的**：自 / 对 / 上 / 下）。
