...
结算窗**前六行**格式固定，顺序是：**谁胡了 → 翻牌 → 自己 → 对家 → 上家 → 下家**。

```
你胡了 · 平胡 2 分 · 自摸                第 1 行：谁胡了 · 牌型几分 · 谁点炮（自摸就写「自摸」）
翻9条 → 翻牌鸡 幺鸡（金）                第 2 行：翻到什么 → 翻牌鸡是哪张（金鸡多一个「（金）」）
清风雀神［叫牌］ 杠6条(自)，胡牌 2        （共 2 分）   第 3~6 行：自己 → 对家 → 上家 → 下家
对家［叫牌］ 幺鸡(金)×1，乌骨鸡×2        （共 4 分）      行尾「（共 N 分）」右对齐
上家［叫牌］ 幺鸡(金)×2，乌骨鸡×2        （共 6 分）
下家［未叫］
豆：… 三家各付 3 分                      再往下：豆 · 未摸到的牌 · 本局 / 累计
```

读的时候只要盯四处：

1. **［叫牌］/［未叫］** —— 只有叫牌家能收；未叫家只付不收，所以他那一行没有「（共 N 分）」。
2. **叫牌家那一行 = 他的杠 + 他的鸡 + 冲锋鸡 + 胡牌**，项之间用「，」分隔：
   - **杠**：`杠9万(自)` —— 括号里是**谁放的**（自 / 对 / 上 / 下）。
   - **鸡**：`幺鸡×2`、`乌骨鸡×1`；金的那张写 `幺鸡(金)×2`；**翻牌鸡直接写牌名**
     （`5筒×1` —— 第 2 行已经说了翻牌鸡是哪张）。张数是**手里 + 副露 + 自己牌河的总数**。
   - **冲锋鸡**：`冲锋鸡×1`；**胡牌**：`胡牌 2`。
3. **行尾的「（共 N 分）」**（右对齐）—— 这一家上面那些牌与胡牌的**分值合计**（按每张的单价算，
   不乘家数）。**它不是本局净额** —— 谁收谁付看侧栏的「本局 ±N」。
4. **包鸡** —— 未叫家打出的 幺鸡 / 8筒 要赔给每个叫牌家，写在他**自己那一行**的尾巴上：
   `→ 包鸡 各 N 分`。
