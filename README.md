...
翻4筒 → 翻牌鸡 5筒                   第 2 行：翻到什么 → 翻牌鸡是哪张（金鸡后面多一个「（金）」）
夜航将友［未叫］                     第 3~6 行：自己 → 对家 → 上家 → 下家
对家［未叫］
上家［叫牌］乌骨鸡×1，5筒×1
下家［叫牌］杠9万(自)，幺鸡×2，5筒×1
豆：… 三家各付 3 分                  再往下：豆 · 冲锋鸡 · 责任鸡 · 未摸到的牌 · 本局 / 累计
```

读的时候只要盯三处：

1. **［叫牌］/［未叫］** —— 只有叫牌家能收；未叫家只付不收，所以**他那一行只写到「［未叫］」为止**
   （他的杠、他的鸡一律不列，列了也都是 0 分）。
2. **叫牌家那一行 = 他的杠 + 他的鸡**，项之间用「，」分隔：
   - **杠**：`杠9万(自)` —— 括号里是**谁放的**（自 / 对 / 上 / 下）。
   - **鸡**：`幺鸡×2`、`乌骨鸡×1`；金的那张写 `幺鸡(金)×2`；**翻牌鸡直接写牌名**
     （`5筒×1` —— 第 2 行已经说了翻牌鸡是哪张）。张数是**手里 + 副露 + 自己牌河的总数**。
3. **包鸡** —— 未叫家打出的 幺鸡 / 8筒 要赔给每个叫牌家，写在他**自己那一行**的尾巴上：
   `→ 包鸡 各 N 分`。
