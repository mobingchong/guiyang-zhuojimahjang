...
| **翻牌鸡**（本局翻牌的下一张） | **1 分/张** |
| **冲锋鸡** | **每个牌种各一只**（1条、8筒）；每只 **2 分/家**，该牌种是金鸡时 **4 分/家** |
| **责任鸡** | 打出的幺鸡被叫牌家碰/杠走 → 打出者**多付 1 分** |
| **谁的鸡算分** | **只有 幺鸡 / 8筒**；**未叫家的鸡一律不算分**（收不到） |

**鸡牌**：胡牌时翻出牌墙第 1 张未摸的牌，**它的下一张就是本局的鸡**（例：翻 4万 → 鸡是 5万）；鸡正好是 **幺鸡 / 8筒** 时该牌种全场 **2 分/张**（见 §5-2）。
...
| 你想知道 | 一句话答案 | 详见 |
|---|---|---|
| 鸡是什么牌、值多少？ | ①**幺鸡(1条)** ②**乌骨鸡(8筒)** ③**翻牌鸡**（翻到的牌的下一张，例：翻 4万 → 鸡是 5万）—— 都是 **1 分/张**；翻到 9条 → 幺鸡 **2 分/张**，翻到 7筒 → 8筒 **2 分/张** | §2 |
...
| **鸡牌** | **翻到的牌的下一张**（点数 +1；9 之后回到 1）。例：翻 4万 → 鸡是 5万。 |
| **翻牌鸡** | 就是上一条的**鸡牌** —— 它**本身也是一类鸡，参与计分**（1 分/张）。 |
| **常鸡** | 幺鸡（1条）、乌骨鸡（8筒）—— 天生就是鸡。 |
| **金鸡** | 本局的**鸡牌**正好是 幺鸡 / 8筒 时，该牌种全场升级（**2 分/张**）：翻 **9条** → 金幺鸡；翻 **7筒** → 金乌骨鸡。 |
...
| **翻牌鸡**（本局翻牌的下一张，例：翻 4万 → 鸡是 5万） | **1 分/张** | （它本身就是 幺鸡/8筒 时，见上两行） |
...
按「这张鸡在谁那儿、处于什么状态」分四种 —— 只认**三类鸡**：**幺鸡(1条)**、**乌骨鸡(8筒)**、**本局翻牌鸡**（翻到的牌的下一张，例：翻 4万 → 鸡是 5万）：
...
- **翻牌定出"本局的鸡"**：**鸡 = 翻到的牌的下一张**（点数 +1；9 之后回到 1）。
  例：翻 **4万** → 鸡是 **5万**；翻 **8条** → 鸡是 **9条**；翻 **9条** → 鸡是 **幺鸡**；翻 **7筒** → 鸡是 **8筒**。
- 本局**能计分的鸡**仍然只有 **幺鸡(1条) / 8筒** 两种：鸡正好翻到它俩之一 → 该牌种全场 **2 分/张**；否则照常 **1 分/张**。
- **翻牌鸡参与计分**：本局的鸡牌（= 翻到的牌的下一张）**全场算鸡、1 分/张** —— 叫牌家手上有几张就收几份；
  未叫家**打出的**照 §4 归叫牌家（包鸡），留着的仍不算。
...
**牌面**：你对家点炮、你**平胡**（1 番）；你另外还有一付**闷豆（暗杠）6条**。
...
```
翻5万　鸡是 6万　· 本局鸡：6万 1 分/张 ＋ 幺鸡(1条) / 8筒（各 1 分/张）  ← 翻到什么 → 鸡是下一张
