...
5. **右上角是全屏按钮**；**右下角是 🔊 音效开关**，**左下角是 🗣 语音开关**（两个开关都会记住状态）。
   **语音默认关闭**，点一下才开；**语音**会用系统语音念出**打出的牌**（如「一万」「六筒」），
   它与音效**互不影响**，可以只开一个。
...
**想赢更多分** → **[实战指南](docs/实战指南.md)**：几万局模拟数据整理的打法（平胡还是大牌、怎么放牌、要不要冲鸡、碰牌取舍）。
...
| **抢杠胡** | **包三家** | 别人**补杠**的那张牌正好被你胡 → 被抢者**一个人付三份**（另外两家不用掏） |
| **热炮**（杠上炮） | **×2** | 点炮者**杠后打出的第一张牌**点了炮 |
...
**热炮的点炮者、抢杠的被抢者**，本局**不能有任何进钱的机会**：

- 他的**鸡**（幺鸡 / 乌骨鸡 / 翻牌鸡 / 冲锋鸡）全不算，**豆**也全部不结算；
- 他该**赔**的照赔 —— 热炮的炮钱（×2）、抢杠的包三家（三份）照给；
- 除这些之外**不再额外赔付**（不倒赔豆、不赔包鸡 / 责任鸡）。
...
| **翻牌鸡** | 本局翻到的牌的**下一张**（例：翻 5万 → **6万** 就是翻牌鸡） | 整局都算 | **1 分/张** |
| **冲锋鸡** | 本局**第一张打出的** 幺鸡 / 8筒（各一只） | 那张牌被打出时 | **2 分/家**；翻到 9条 / 7筒 → 该只 **4 分/家** |
| **责任鸡** | 打出的 **幺鸡** | 被**叫牌家**碰 / 杠走时 | 打出者**多付 1 分** |

- **幺鸡 / 乌骨鸡 / 翻牌鸡** 这三种算鸡；**叫牌家的**（手牌 + 副露 + 自己牌河）有几张算几张，
  **未叫家的鸡一律不算分**（收不到）。
- **金**：本局的鸡（就是翻牌鸡）正好是 **幺鸡 / 8筒** 时，该牌种本局 **2 分/张**
  （翻 **9条** → 金幺鸡；翻 **7筒** → 金乌骨鸡）。这时候翻牌鸡本身就是幺鸡/8筒，按 2 分算、不再另加一份。
- **包鸡只认 幺鸡(1条) / 8筒**：未叫家**打出的**翻牌鸡不赔。
- **冲锋鸡 / 责任鸡** 只认 **幺鸡 / 8筒**，与上面那套**分开单独算**。

**黄庄**：豆、鸡**都不算分**；但要**查叫** —— 未叫牌的人赔给每个叫牌者「其叫牌牌型的分值」。
...
| 你想知道 | 一句话答案 | 详见 |
|---|---|---|
| 鸡是什么牌、值多少？ | **幺鸡(1条)**、**乌骨鸡(8筒)**、**翻牌鸡**（翻到的牌的下一张，例：翻 5万 → 6万）各 **1 分/张**；翻到 9条 → 幺鸡 2 分/张，翻到 7筒 → 8筒 2 分/张 | 「牌型与分值」§4 鸡分 |
...
| **本局的鸡** | **翻到的牌的下一张**（点数 +1；9 之后回到 1）。例：翻 5万 → 鸡是 6万。 |
| **翻牌鸡** | **本局的鸡** —— 它本身就是一类**算分的鸡**（1 分/张）。 |
| **常鸡** | 幺鸡（1条）、乌骨鸡（8筒）—— 天生就是鸡。 |
| **金鸡** | 本局的鸡（翻牌鸡）正好是 幺鸡 / 8筒 时该牌种的叫法：翻 **9条** → 金幺鸡；翻 **7筒** → 金乌骨鸡。 |
| **冲锋鸡** | **每个牌种各一只**：本局**第一张**被打出的**幺鸡**、**第一张**被打出的**乌骨鸡**。两只属同一家就合并算。 |
| **责任鸡** | 打出的**幺鸡**被**叫牌家碰（或明杠）走**时，打出者要多付的那 1 分。**乌骨鸡不算**。 |
| **豆腐** | 杠。**闷豆**＝暗杠、**爬坡豆**＝补杠、**点豆**＝明杠（别人放的第 4 张）。 |
...
按「这张鸡在谁那儿、处于什么状态」分四种 —— 只认**三种鸡**：**幺鸡(1条)**、**乌骨鸡(8筒)**、**本局的翻牌鸡**：
...
- **什么时候翻**：有人胡牌、且牌墙还有牌 → 翻牌墙第 1 张未摸的牌；牌墙已空（黄庄）→ 不翻。
- **翻牌定出「翻牌鸡」**：**翻牌鸡 = 翻到的牌的下一张**（点数 +1；9 之后回到 1），它**算 1 分/张**。
  例：翻 **5万** → 翻牌鸡是 **6万**；翻 **8条** → **9条**；翻 **9条** → **幺鸡**；翻 **7筒** → **8筒**。
- **金鸡**：翻牌鸡正好是 **幺鸡 / 8筒** 之一 → 该牌种本局 **2 分/张**（翻 **9条** → 金幺鸡；翻 **7筒** → 金乌骨鸡）。
- **包鸡只认 幺鸡 / 8筒**：未叫家打出的翻牌鸡**不赔**。
...
```
一局总分 = 牌型分值（**取最高**；自摸 +1、杠上花 +1、抢杠＝包三家、热炮 ×2）
...
| 家 | 状态 | 鸡（手里的 / 打出的） | 他算的鸡数 |
|---|---|---|---|
| **你**（0） | 胡牌 → **叫牌家** | 无 | **0** |
| 下家（1） | 未叫牌 | 幺鸡×1 | **0**（未叫家收不到） |
| 对家（2） | 叫牌 → **叫牌家** | 幺鸡×1、乌骨鸡×2 | **3** |
| 上家（3） | 叫牌 → **叫牌家** | 幺鸡×2、乌骨鸡×2 | **4** |
...
结算窗的行顺序固定：**谁胡了 → 翻牌 → 自己 → 对家 → 上家 → 下家 → 胡牌收付 → 杠**，
右边那栏是四家的本局净额与累计积分（同样是 **自己 → 对家 → 上家 → 下家**）。

```
你胡了 · 平胡 2 分 · 自摸                第 1 行：谁胡了 · 牌型几分 · 谁点炮（自摸就写「自摸」）
翻9条 → 翻牌鸡 幺鸡（金）                第 2 行：翻到什么 → 翻牌鸡是哪张（金鸡多一个「（金）」）
清风雀神［叫牌］ 杠6条(自)                （3 个）   第 3~6 行：自己 → 对家 → 上家 → 下家
对家［叫牌］ 幺鸡(金)×1，乌骨鸡×2         （4 个）      行尾「（N 个）」右对齐
上家［叫牌］ 幺鸡(金)×2，乌骨鸡×2         （6 个）
下家［未叫］
三家→清风雀神自摸（2 个）             胡牌收付：谁付给谁 · 多少分
杠牌 三家→清风雀神（3 个）            杠的收付：谁付给谁 · 每个豆 3 分
清风雀神 本局 +5 分（收 15 − 付 10）     右栏：本局净额 + 收付算式
对家 本局 +1 分（收 12 − 付 11）
上家 本局 +9 分（收 18 − 付 9）
下家 本局 -15 分（收 0 − 付 15）
累计积分 / 未摸到的牌 / 再来一局
...
1. **［叫牌］/［未叫］** —— 只有叫牌家能收；未叫家只付不收，所以他那一行通常只到「［未叫］」。
2. **叫牌家那一行 = 他的杠 + 他的鸡**，项之间用「，」分隔：
   - **杠**：`杠9万(自)` —— 括号里是**谁放的**（自 / 对 / 上 / 下）。
   - **鸡**：`幺鸡×2`、`乌骨鸡×1`；金的那张写 `幺鸡(金)×2`；**翻牌鸡直接写牌名**（`5筒×1`）；
     **冲锋鸡**写 `冲锋鸡×1`。张数是**手里 + 副露 + 自己牌河的总数**。
3. **行尾「（N 个）」（右对齐）** —— 这一家**名下的单位值** = 鸡 + 杠（每个豆 3）。
   ⚠️ 这里的单位是「**个**」、不是「分」：它要 **×3**（未叫家的负数则 ×叫牌家数）才是真正收付的钱，
   所以跟右栏结算出来的「分」区分开。胡牌那几个**不在这里**（单独一行）；未叫家持有冲锋鸡时是负数。
4. **胡牌收付行** —— `上家点炮→下家胡牌（1 个）`；自摸写 `三家→X自摸（2 个）`；
   抢杠写 `上家补杠被抢→下家胡牌（包三家 3 个）`，热炮写 `上家热炮（杠上炮）→下家胡牌（2 个）`。
   **热炮 / 抢杠的责任人**行尾标 **`· 一废全废`**（他的鸡不算分、豆不结算，共分记 0）。
5. **杠的收付行** —— `杠牌 三家→X（3 个）` 是自杠（其他三家各付 3）、
   `杠牌 上家→X（3 个）` 是点豆（只有放杠那家付）；杠家没叫牌时反过来倒赔。

右栏每家后面的 **`（收 X − 付 Y）`** 是这一局他所有收入与付出的合计 ——
**收 − 付 = 本局净额**，一笔一笔都对得上；窄屏放不下时会折到下一行。
